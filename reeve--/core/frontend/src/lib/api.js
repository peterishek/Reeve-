...
// Local-check-first recipient preview for LOCAL transfers — resolves
// against Reeve's own DB (no external API call at all), the same way
// resolveAccount() below checks Paystack for external transfers.
export function lookupAccount(accountId) {
  return apiFetch(`/api/accounts/lookup/${accountId}/`);
}
...
// ---------- External (domestic, Paystack-verified) transfers ----------
...
// `accountName` MUST be the value returned by resolveAccount() above —
// never something typed by the user — or the "verified" badge is a lie.
export function saveBeneficiary({ bankId, accountNumber, accountName }) {
  return apiFetch("/api/beneficiaries/", {
    method: "POST",
    body: JSON.stringify({ bank_id: bankId, account_number: accountNumber, account_name: accountName }),
  });
}

export function externalTransfer({ fromAccountId, beneficiaryId, amount, description, idempotencyKey }) {
  return apiFetch("/api/transfer/external/", {
    method: "POST",
    body: JSON.stringify({
      from_account_id: fromAccountId,
