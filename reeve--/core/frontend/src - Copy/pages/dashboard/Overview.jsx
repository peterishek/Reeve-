...
import { Send, CreditCard, FileDown, ArrowUpRight, ArrowDownLeft, Globe } from "lucide-react";
    ...
    <div>
      ...
      {activity.length === 0 ? (
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${c.paperDeep}` }}>
          {activity.map((txn, i) => {
            const isInternational = txn.transfer_type === "international";
            return (
              <div
                key={txn.id}
              ...
              >
                <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0" style={{ background: c.paper }}>
                  {isInternational ? (
                    <Globe className="w-4 h-4" style={{ color: c.cobalt }} />
                  ) : (
                    <ArrowUpRight className="w-4 h-4" style={{ color: c.cobalt }} />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="f-body text-sm font-semibold truncate" style={{ color: c.ink }}>
                    {isInternational
                      ? `To ${txn.international_detail?.recipient_name || "external recipient"}`
                      : txn.description || "Transfer"}
                  </div>
                  <div className="f-body text-[11px]" style={{ color: c.slate }}>
                    {isInternational ? "International" : "Local"} · {new Date(txn.created_at).toLocaleString()}
                  </div>
                </div>
